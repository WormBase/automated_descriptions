...
my $gr_ec_dir = $home . "gene_regulation_expression_cluster/";
my $elegans_gene_list_dir = $home_elegans . "gene_lists/" ;
my $gene_list_dir = $home . "gene_lists/" ;
my $input_path = $gr_ec_dir . "input_files/";
my $output_path = $gr_ec_dir . "output_files/";
my $gene_dir = $output_path . "individual_gene_sentences/";
my $input_file = $input_path . $prefix . "ECsummary_geneReg.$RELEASE.txt";
...
my $output_log = $output_path . "sentences_for_gene_regulation_expression_cluster.txt";
# if the output file exists delete it.
if (-e $output_log){
   my @args = ("/bin/rm", "-f", $output_log);
   system(@args) == 0 or die("could not delete file $output_log\n");
}
my $output_test_log = $output_path . "test_sentences_for_gene_regulation_expression_cluster.txt";
# if the output file exists delete it.
if (-e $output_test_log){
   my @args = ("/bin/rm", "-f", $output_test_log);
   system(@args) == 0 or die("could not delete file $output_test_log\n");
}
my $output = $output_path . "gene_regulation_expression_cluster.txt";
my $output_test = $output_path . "test_gene_regulation_expression_cluster.txt";
my $output_genes = $output_path . "gene_ids_gene_regulation_expression_cluster.txt";
my $output_test_genes = $output_path . "test_gene_ids_gene_regulation_expression_cluster.txt";
# if the output file exists delete it.
if (-e $output){
