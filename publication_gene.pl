...
my $output = $gene_list_dir . "textpresso_gene_popularity.text";
if (-e $output){
  my @args = ("rm", "-f", $output);
  system(@args) == 0 or die("could not delete file $output\n");
}
my $header="id\tname\tpapers\n";
...
sub get_papers {
 ...
 my $gene = shift;
 my @papers=();
 my @sections = qw(body abstract acknowledgments references accession conclusion discussion introduction materials results title);
