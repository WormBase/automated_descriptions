...
my $download_script = "./download_gene_associations_parallel.pl";
...
foreach my $line (@lines){
 ...
 $project =~s/^\s+//;
 $project =~s/\s+$//;
 my $term = $species . "AND" . $project;
 push(@species_array, $term);
}

 @args=($parallel_exec,  $j_flag, $percent, $download_script, $three_colons, @species_array);
 $status=system(@args);
