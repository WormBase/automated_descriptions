...
my $local_file = $orthology_dir . $species . ".$RELEASE.xrefs.txt.gz";
if (-e $local_file){
...
my $unzipped_file;
#
my $output_file = $orthology_dir . $species . ".xrefs.txt";
if (-e $output_file){
   @args = ("/bin/rm", "-f", $output_file);
   system(@args) == 0 or die("could not delete file $output_file\n");
}
#
my $url = "ftp://ftp.sanger.ac.uk/pub/wormbase/releases/$RELEASE/species/$species/$project/$species.$project.$RELEASE.xrefs.txt.gz";
#
